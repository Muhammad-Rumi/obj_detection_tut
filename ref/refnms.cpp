...
vector<float> GetPointFromRect(const vector<vector<float>>& rect,
  ...
  for (const auto& p : rect) points.push_back(p[pos]);

  return points;
}

vector<float> ComputeArea(const vector<float>& x1, const vector<float>& y1,
...
template <typename T>
vector<T> FilterVector(const vector<T>& vec, const vector<int>& idxs) {
  vector<T> resultVec;
